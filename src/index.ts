...
function createFilterFunction(filters: Filters): (data: DataArray) => DataArray {
  ...
  const fnString = "filterFunction = function (data) {"
    + "	var result = [], obj;"
...
function getValue(obj: DataItem, key: string): any {
  let context: any = obj
    , bail = false
    , keys = key.split('.')
    , thisKey: string | null = null;
...
class Selector {
  ...
  executeFilter(data: DataItem | DataArray): DataArray {
    ...
    return this.filterFunction(data);
  }
  ...
  filter(obj?: Filter | Filter[]): Filter[] | Selector {
    this.filterFunction = null;
